      ...
      <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <enf_material_usado
              :materiales="saldo_semana"
              :presente="presente != 0"
              :futuro="futuro != 0"
              :datosenfunde="lote_enfunde"
            ></enf_material_usado>
          </div>
      ...
      <div class="col-md-6">
        <div class="col-12 table-responsive">
          ...
          <table class="table table-bordered table-hover" id="enfunde-items">
            ...
            <tbody>
              <tr v-for="(sec, index) in seccion" class="text-center" :key="index">
                <td width="10%">
                  <span v-if="status">
    ...
    <div class="row">
      <div class="col-12" v-if="fundas.length > 0">
        ...
        <table class="table table-bordered table-hover" id="despacho-items">
          ...
          <tbody>
            <tr v-for="(funda, index) in fundas" class="text-center" :key="index">
              <td>{{funda.fecha}}</td>
      ...
      <div class="col-12" v-if="reemplazos.length > 0">
        ...
        <table class="table table-bordered table-hover" id="reemplazo-items">
          ...
          <tbody>
            <tr v-for="(reemplazo, index) in reemplazos" class="text-center" :key="index">
...
export default {
    ...
    $("#btn-save").on("click", function() {
      if (self.status) {
        ...
        if (+self.totalEnfunde() > 0) {
          ...
          axios
            .post("/sistema/enfunde/registro/save", {
  ...
  methods: {
              ...
              if (datos4) {
                ...
                for (var y in datos4.detalle) {
                  if (datos4.detalle[y].idseccion == seccion.seccion) {
                    ...
                    seccion.presente.cantidad += +datos4.detalle[y]
                      .cant_presente;
              ...
              for (var i in datos3.egresos) {
                if (datos3.egresos[i].reemplazo == 0) {
                  var despacho = {
                    ...
                    status:
                      datos3.egresos[i].presente == 1 ? "Presente" : "Futuro"
                  };
    ...
    editForm: function(index, b = false) {
      ...
      this.lote_enfunde.presente.cantidad = this.seccion[
        index
      ].presente.cantidad;
      this.lote_enfunde.presente.materiales = this.seccion[
        index
      ].presente.materiales;
      this.lote_enfunde.futuro.cantidad = this.seccion[index].futuro.cantidad;
      this.lote_enfunde.futuro.materiales = this.seccion[
        index
      ].futuro.materiales;
    ...
    saveForm: function(index) {
      ...
      this.seccion[
        index
      ].presente.cantidad = this.lote_enfunde.presente.cantidad;
      this.seccion[
        index
      ].presente.materiales = this.lote_enfunde.presente.materiales;
      this.seccion[index].futuro.cantidad = this.lote_enfunde.futuro.cantidad;
      this.seccion[
        index
      ].futuro.materiales = this.lote_enfunde.futuro.materiales;
    ...
    saldoEnfunde: function(
      ...
      if (saldos) {
        if (saldos.semana) {
          if (edicion) {
            saldos.semana.saldo =
              parseInt(saldos.semana.saldo) + +saldo_edicion;
          }
          ...
          if (saldos.presente) {
            if (saldos.futuro) {
              if (this.presente) {
                if (edicion) {
                  saldos.presente.saldo =
                    parseInt(saldos.presente.saldo) + +saldo_edicion;
                }
                return (saldos.presente.saldo =
                  saldos.presente.saldo - +actualiza_saldo);
              } else {
                if (edicion) {
                  saldos.futuro.saldo =
                    parseInt(saldos.futuro.saldo) + +saldo_edicion;
                }
                return (saldos.futuro.saldo =
                  saldos.futuro.saldo - +actualiza_saldo);
              }
            }
          }
        }
    ...
    graficarLotes: function() {
      ...
      for (var x in this.seccion) {
        Presente.data.push(
          parseFloat(
            +this.seccion[x].presente.cantidad / +this.seccion[x].has
          ).toFixed(0)
        );
        ...
        Futuro.data.push(
          parseFloat(
            +this.seccion[x].futuro.cantidad / +this.seccion[x].has
          ).toFixed(0)
        );
